...
function descendantIds(nodes, id) {
  ...
  while (stack.length) {
    ...
    for (const child of nodes.filter((n3) => n3.parent === current)) {
...
async function callUpdateItem(hass, data) {
...
async function saveStructure(hass, structure) {
...
const STR = {
  ...
  noParentsAvailable: (parentLevelLabel) => `Сначала добавьте узел уровня «${parentLevelLabel}»`,
  ...
  desyncWarning: "Существующие товары в этом месте сохранят старый путь; при необходимости обновите их вручную.",
  confirmDeleteTitle: (name) => `Удалить «${name}»?`,
  cascadeWarning: (n3) => `Будет удалено ${n3} узлов вместе с этим.`,
  ...
  migrationOfferText: (oldPath, newPath) => `Путь изменился: «${oldPath}» → «${newPath}». Обновить места товаров, сохранённых по старому пути?`,
  migrationSearching: "Поиск товаров…",
  migrationFound: (n3) => n3 === 0 ? "Подходящих товаров не найдено." : `Найдено товаров для обновления: ${n3}.`,
...
function siblingNameTaken(nodes, level, parentId, name, excludeId) {
  return nodes.some(
    (n3) => n3.level === level && n3.parent === (parentId ?? null) && n3.name === name && n3.id !== excludeId
  );
}
function migrateLocationString(locationStr, oldSegments, newSegments) {
  ...
  for (let i2 = 0; i2 < oldSegments.length; i2++) {
    if (segs[i2] !== oldSegments[i2]) return locationStr;
  }
  const remainder = segs.slice(oldSegments.length);
  return buildPath([...newSegments, ...remainder]);
}
class InventoryStructureCard extends i {
  constructor() {
    super();
  ...
  // --- Add form ----------------------------------------------------------
  _onLevelChange(e2) {
    this._addLevel = Number(e2.target.value);
    this._addParentId = null;
    this._error = null;
  }
  async _onAddSubmit() {
  ...
  _renderAddForm() {
    ...
    const parentCandidates = level === LEVEL.ROOM ? [] : this._structure.nodes.filter((n3) => n3.level === parentLevel);
    const sortedParents = [...parentCandidates].sort(
      (a2, b2) => pathOfNode(this._structure.nodes, a2.id).localeCompare(pathOfNode(this._structure.nodes, b2.id), "ru")
    );
    ...
    return b`
      <div class="row wrap">
        <div>
          ...
          <select .value=${String(level)} @change=${(e2) => this._onLevelChange(e2)}>
            ${[LEVEL.ROOM, LEVEL.FURNITURE, LEVEL.SHELF].map(
      (l2) => b`<option value=${l2}>${LEVEL_LABELS_RU[l2]}</option>`
    )}
          </select>
        </div>
        ${level !== LEVEL.ROOM ? b`
              <div>
                ...
                <select
                  ...
                  @change=${(e2) => {
      this._addParentId = e2.target.value || null;
    }}
                >
                  <option value="">${STR.selectParent}</option>
                  ${sortedParents.map(
      (n3) => b`<option value=${n3.id}>${pathOfNode(this._structure.nodes, n3.id)}</option>`
    )}
                </select>
              </div>
            ` : ""}
      ...
      ${blockedByNoParents ? b`<div class="muted">${STR.noParentsAvailable(LEVEL_LABELS_RU[parentLevel])}</div>` : ""}
  ...
  async _saveRename(node) {
    ...
    if (this._editName !== node.name && siblingNameTaken(this._structure.nodes, node.level, node.parent, this._editName, node.id)) {
    ...
    const newNodes = this._structure.nodes.map((n3) => n3.id === node.id ? { ...n3, name: this._editName } : n3);
  ...
  async _confirmDelete(node) {
    ...
    const idsToRemove = /* @__PURE__ */ new Set([node.id, ...descendants]);
    const newNodes = this._structure.nodes.filter((n3) => !idsToRemove.has(n3.id));
    const newStructure = { version: 1, nodes: newNodes };
  ...
  async _startMigrationPreview() {
    ...
    try {
      ...
      for (const item of items) {
        const locs = Array.isArray(item.locations) && item.locations.length ? item.locations : item.location ? [item.location] : [];
        if (!locs.length) continue;
        const migrated = locs.map((l2) => migrateLocationString(l2, oldSegments, newSegments));
        const changed = migrated.some((l2, i2) => l2 !== locs[i2]);
        if (changed) {
  ...
  async _confirmMigration() {
    const m2 = this._migration;
    if (!m2 || !m2.matches) return;
    this._migration = { ...m2, busy: true };
    let updated = 0;
    for (const { item, newLocation } of m2.matches) {
      try {
    ...
    this._migration = { ...m2, busy: false, done: true, updated, total: m2.matches.length };
    this._migrationOffer = null;
  }
  ...
  _renderMigrationPanel() {
    const m2 = this._migration;
    if (!m2) return "";
    if (m2.busy && !m2.matches && !m2.done) {
      return b`<div class="banner muted">${STR.migrationSearching}</div>`;
    }
    if (m2.error) {
      return b`
        <div class="banner">
          <span class="error-text">${m2.error}</span>
          <button class="btn" @click=${() => this._closeMigrationPanel()}>${COMMON.close}</button>
        </div>
      `;
    }
    if (m2.done) {
      const failed = m2.total - m2.updated;
      return b`
        <div class="banner">
          <span>${STR.migrationDone(m2.updated, m2.total)}${failed > 0 ? STR.migrationPartialFail(failed) : ""}</span>
          <button class="btn" @click=${() => this._closeMigrationPanel()}>${COMMON.close}</button>
        </div>
      `;
    }
    if (m2.matches) {
      return b`
        <div class="banner">
          <span>${STR.migrationFound(m2.matches.length)}</span>
          ${m2.matches.length > 0 ? b`<button class="btn btn-primary" ?disabled=${m2.busy} @click=${() => this._confirmMigration()}>
                ${COMMON.confirm}
              </button>` : ""}
  ...
  _renderNodeRow(node, depth) {
    ...
    return b`
      ...
      this._editName = e2.target.value;
            ...
            `}
        ${isEditing ? b`<div class="confirm-box">
              <span class="muted">${STR.desyncWarning}</span>
              ${this._editError ? b`<span class="error-text">${this._editError}</span>` : ""}
            </div>` : ""}
  ...
  _renderDeleteConfirm(node) {
    ...
    return b`
      <div class="confirm-box">
        ...
        ${descendants.length > 0 ? b`<span class="error-text">${STR.cascadeWarning(descendants.length)}</span>` : ""}
        <span class="muted">${STR.desyncWarning}</span>
        ${this._deleteError ? b`<span class="error-text">${this._deleteError}</span>` : ""}
        <div class="row">
  ...
  render() {
    ...
    if (!this._structure) {
      return b`
        <ha-card>
          ${((_a2 = this._config) == null ? void 0 : _a2.title) ? b`<div class="card-header">${this._config.title}</div>` : ""}
    ...
    return b`
      <ha-card>
        ${((_b = this._config) == null ? void 0 : _b.title) ? b`<div class="card-header">${this._config.title}</div>` : ""}
        ${this._renderAddForm()}
        <hr class="divider" />
        ${this._renderMigrationOffer()}
        ${this._renderMigrationPanel()}
        <div class="tree">
          ${tree.length ? tree.map((node) => this._renderNodeRow(node, 0)) : b`<div class="empty-state">${STR.emptyTree}</div>`}
        </div>
      </ha-card>
    `;
  }
}
__publicField(InventoryStructureCard, "properties", {
...
__publicField(InventoryStructureCard, "styles", [
  sharedStyles,
  i$3`
      .node-row {
        display: flex;
